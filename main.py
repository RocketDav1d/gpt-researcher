...
GOOGLE_API_DEVELOPER_KEY=os.environ['GOOGLE_API_DEVELOPER_KEY']
GOOGLE_SEARCH_API_ID=os.environ['GOOGLE_SEARCH_API_ID']
...
def summarize(query, res, link, add_note):
    ...
    for chunk in response:
        if chunk.choices[0].delta.content is not None:
            content = chunk.choices[0].delta.content
            output += content  # This line might be causing duplication
            print(content, end="")
            if "}" in content:
                break  # Break once the closing brace of JSON is found
    ...
    print("summarise return", output.strip())
    return output.strip()
...
def create_summaries(search_query, search_info, query_links, ordered_links, json_dict, json_data, new_key, i):
    ...
    link_count = 1
    # Initialize a summary counteask_questionr
    summary_count = 0
    ...
    while link_verdict_json['continue'] and not all_links_processed and summary_count <= 5:
        ...
        for ordered_link in ordered_links:
            ...
            for segment in segments:
                link_summary = summarize(query=search_info, res=segment, link=current_link, add_note=reason)
